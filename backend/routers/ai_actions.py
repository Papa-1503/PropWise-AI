...
MULTI-TENANCY: real, previously-live gaps closed here - list_actions
and decide_action had NO org filter at all, meaning any staff member
could view, approve, reject, or edit ANY other organization's AI
Actions. generate_actions now stamps every created action with a real
orgId, gather_portfolio_context and occupancy_insight are scoped by it
(previously aggregated every organization's leases/tickets/payments
combined when no specific property was selected), and
_do_auto_approve_check now loops per organization rather than
auto-approving eligible actions across every org's suggested queue at
once.
"""
import json
import os
...
from db import ai_actions_col, leases_col, tickets_col, properties_col, payments_col, organizations_col
...
async def gather_portfolio_context(org_id: str, property_id: str | None) -> str:
    """
    ...
    MULTI-TENANCY: org_id is required and always applied - without it,
    the portfolio-wide case (no specific property selected) would
    gather leases/properties/tickets/payments across every
    organization in the database combined.
    """
    prop_filter: dict = {"orgId": org_id}
    if property_id:
        prop_filter["propertyId"] = property_id
    sections = []
...
async def generate_actions(propertyId: str | None = None, user: dict = Depends(require_staff)):
    ...
    context_text = await gather_portfolio_context(user["orgId"], propertyId)
...
async def execute_renewal_campaign(action: dict):
    ...
    leases = await leases_col.find(
        {"propertyId": action.get("propertyId"), "unitId": {"$in": unit_ids}, "orgId": action.get("orgId")}
    ).to_list(length=200)
...
async def execute_collections_reminder(action: dict):
    ...
    delinquent_charges = await payments_col.find(
        {"propertyId": action.get("propertyId"), "unitId": {"$in": unit_ids}, "dueDate": {"$lt": now}, "orgId": action.get("orgId")}
    ).to_list(length=200)
    owed_by_unit = {}
...
async def occupancy_insight(propertyId: str | None = None, user: dict = Depends(require_staff)):
    ...
    """
    prop_filter: dict = {"orgId": user["orgId"]}
    if propertyId:
...
async def _do_approve_action(action_id: str, decided_by_email: str, decided_by_user_id: str | None, org_id: str | None = None) -> dict:
    ...
    MULTI-TENANCY: org_id is optional here since the auto-approval path
    below already scopes its own candidate query by org before ever
    calling this - but when a real org_id IS passed (the manual
    decide_action path), it's checked as real defense in depth, so a
    staff member can never approve a different organization's action
    even by guessing its real ID."""
    updates = {
        "status": "executing",
    ...
    query: dict = {"_id": ObjectId(action_id)}
    if org_id:
        query["orgId"] = org_id
...
async def _do_auto_approve_check():
    ...
    MULTI-TENANCY: loops per organization, same real pattern as every
    scheduled check in routers/admin.py - without this, the query
    below would auto-approve eligible actions across every
    organization's suggested queue combined."""
    ...
    for org in orgs:
        ...
        for action in candidates:
            result = await _do_approve_action(
                str(action["_id"]), decided_by_email="system_auto_approval", decided_by_user_id=None, org_id=org_id,
            )
            if result:
                approved.append(str(action["_id"]))

    return {"status": "done", "candidatesChecked": candidates_checked, "autoApproved": len(approved), "actionIds": approved}
...
async def decide_action(action_id: str, payload: ActionDecision, user: dict = Depends(require_staff)):
    ...
    # MULTI-TENANCY: real, previously-live gap closed here - without
    # this org check, a staff member could approve, reject, or edit a
    # DIFFERENT organization's suggested action simply by knowing (or
    # guessing) its real ID.
    action = await ai_actions_col.find_one({"_id": ObjectId(action_id), "orgId": user["orgId"]})
    if not action:
    ...
    if payload.decision == "reject":
        ...
        result_doc = await _do_approve_action(action_id, decided_by_email=user.get("email"), decided_by_user_id=user.get("id"), org_id=user["orgId"])
        return serialize(result_doc)

    result_doc = await ai_actions_col.find_one_and_update(
        {"_id": ObjectId(action_id), "orgId": user["orgId"]}, {"$set": updates}, return_document=True
    )
    return serialize(result_doc)
